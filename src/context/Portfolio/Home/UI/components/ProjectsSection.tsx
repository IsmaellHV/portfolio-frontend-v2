      ...
      <div className="space-y-1">
        {projects.map((project, index) => (
          <a
            ...
            className="group block py-6 -mx-4 px-4 rounded-md hover:bg-muted/30 transition-all duration-300"
          >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span 
                        key={tag} 
                        className="text-xs px-2.5 py-1 rounded-full bg-secondary/50 text-foreground/70 font-mono border border-border/40 group-hover:border-border/60 transition-colors"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            {index < projects.length - 1 && (
              <div className="mt-6 border-b border-border/30 group-hover:border-border/50 transition-colors" />
            )}
          </a>
        ))}
      </div>
