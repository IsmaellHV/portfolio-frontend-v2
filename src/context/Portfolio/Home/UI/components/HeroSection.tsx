...
export const HeroSection = ({ profile }: { profile: Profile }) => {
  return (
    <section id="hero" className="space-y-12 animate-in fade-in slide-in-from-left-4 duration-1000">
      <div className="space-y-6">
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-foreground">
          {profile.name.split(' ')[0]}<br/>
          <span className="text-foreground/50">{profile.name.split(' ')[1]}</span>
        </h1>
        <h2 className="text-base md:text-lg text-foreground/70 font-medium max-w-sm">
          {profile.role}
        </h2>
      </div>
